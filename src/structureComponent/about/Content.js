...
function Content() {
  ...
  const handleScroll = () => {
    ...
    if (journeyElem) {
      ...
      const navigationLis = document.querySelectorAll(
        ".navigation_box ul.navigation li"
      );
    ...
    <div class="content_wrap">
      <div className={`navigation_box ${isNavFixed ? "fixed" : ""}`}>
        <ul class="navigation list-group">
          {["belief", "ingredient", "innovation", "created", "journey"].map(
            (section) => (
              <li key={section}>
                <a
                  href={`#${section}`}
                ...
                >
                  {section === "created"
                    ? section.toUpperCase() + " BY SULWHASOO"
                    : "OUR " + section.toUpperCase()}
                </a>
              </li>
            )
          )}
